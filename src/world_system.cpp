...
bool WorldSystem::step(float elapsed_ms_since_last_update) {
	...
	if (advancedMode) {
		...
		if (registry.hardShells.components.size() <= 0 && next_turtle_spawn < 0.f) {
			// Reset timer
	...
	if (registry.softShells.components.size() <= MAX_FISH && next_fish_spawn < 0.f) {
		...
		motion.position =
			vec2(1500.f -200.f, 
				 50.f + uniform_dist(rng) * (800.f - 100.f));
		// A2: setting a random vertical velocity for fishes
		float random = rand() % 300 - 100;
		motion.velocity = vec2(-100.f, random);
	}
	...
	for (Entity entity : registry.lightups.entities) {
		...
		// lights out once the LightUp timer expired
		if (counter.counter_ms < 0) {
...
void WorldSystem::restart_game() {
	...
	// Create a new salmon
	player_salmon1 = createSalmon(renderer, { 100, 200 });
	registry.colors.insert(player_salmon1, {1, 0.8f, 0.8f});
		...
		if (registry.players.has(entity)) {
			...
			if (registry.hardShells.has(entity_other)) {
				...
				if (!registry.deathTimers.has(entity)) {
					...
					registry.colors.get(entity) = vec3(1.0f, 0, 0);
				}
			}
...
void WorldSystem::on_key(int key, int, int action, int mod) {
	...
	Motion& motion = registry.motions.get(player_salmon1);
	...
	if (action == GLFW_PRESS && key == GLFW_KEY_Z) {
		...
		std::cout << updateFrequency << std::endl;
	}
...
void WorldSystem::on_mouse_move(vec2 mouse_position) {
	...
	// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	Motion& motion1 = registry.motions.get(player_salmon1);
	motion1.angle = atan2(mouse_position[1] - motion1.position[1], mouse_position[0] - motion1.position[0]);
}
